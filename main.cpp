...
#include <ethcoreminer/buildinfo.h>
...
bool g_exitOnError = false;  // Whether or not ethcoreminer should exit on mining threads errors
...
#if ETH_DBUS
#include <ethcoreminer/DBusInt.h>
#endif
...
#undef BACKTRACE_MAX_FRAMES
    ...
    bool validateArgs(int argc, char** argv)
    {
        ...
        CLI::App app("ethcoreminer - GPU Ethash miner");
    ...
    void help()
    {
        cout << "ethcoreminer - GPU ethash miner" << endl
             << "minimal usage : ethcoreminer [DEVICES_TYPE] [OPTIONS] -P... [-P...]" << endl
             << endl
             << "Devices type options :" << endl
             << endl
             << "    By default ethcoreminer will try to use all devices types" << endl
             ...
             << "                        ethcoreminer --help-ext con" << endl
             << endl
    ...
    void helpExt(std::string ctx)
    {
        ...
        if (ctx == "api")
        {
            cout << "API Interface Options :" << endl
                 << endl
                 << "    ethcoreminer provide an interface for monitor and or control" << endl
        ...
        if (ctx == "misc")
        {
            cout << "Miscellaneous Options :" << endl
                 ...
                 << "                        Sets the number of minutes ethcoreminer can stay" << endl
                 ...
                 << "    --exit              FLAG Stop ethcoreminer whenever an error is encountered"
                 << endl
                 << "    --ergodicity        INT[0 .. 2] Default = 0" << endl
                 << "                        Sets how ethcoreminer chooses the nonces segments to"
                 << endl
                 << "                        search on." << endl
        ...
        if (ctx == "con")
        {
            cout << "Connections specifications :" << endl
                 ...
                 << "    the fail-over disconnects ethcoreminer passes to the next connection" << endl
                 << "    available and so on till the list is exhausted. At that moment" << endl
                 << "    ethcoreminer restarts the connection cycle from the first one." << endl
                 << "    An exception to this behavior is ruled by the --failover-timeout" << endl
                 << "    command line argument. See 'ethcoreminer -H misc' for details." << endl
                 << endl
                 << "    The special notation '-P exit' stops the failover loop." << endl
                 << "    When ethcoreminer reaches this kind of connection it simply quits." << endl
                 << endl
                 << "    When using stratum mode ethcoreminer tries to auto-detect the correct" << endl
...
int main(int argc, char** argv)
{
    ...
    auto* bi = ethcoreminer_get_buildinfo();
    cout << endl
         << endl
         << "ethcoreminer " << bi->project_version << endl
    ...
    if (argc < 2)
    {
        cerr << "No arguments specified. " << endl
             << "Try 'ethcoreminer --help' to get a list of arguments." << endl
             << endl;
        return 1;
    }
    ...
    try
    {
        ...
        catch (std::invalid_argument& ex1)
        {
            cerr << "Error: " << ex1.what() << endl
                 << "Try ethcoreminer --help to get an explained list of arguments." << endl
                 << endl;
            return 1;
        }
